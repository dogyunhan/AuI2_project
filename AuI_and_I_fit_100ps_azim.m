...
files = struct();
files.dat      = fullfile(base_path, "AuI2_30mM_0002", "PEPCed_dat.dat");
files.std      = fullfile(base_path, "AuI2_30mM_0002", "PEPCed_std.dat");
...
raw_sads     = readmatrix(files.sads);
raw_sads_std = readmatrix(files.sads_std);
...
dat_100ps = raw_dat(mask, 3);     % 실험 데이터 (100 ps)
std_100ps = raw_std(mask, 3);     % 오차 (Error bar)
...
heat_100ps = raw_solv(mask_solv, 3); 

% PEPC용 Basis Set 구성: [HeatingSignal, Constant, 1/q]
heat_100ps_baseline = [heat_100ps, ones(size(q_fit)), 1./q_fit];

% 2.4. Apply PEPC to Target Data (Optional but recommended)
% 실험 데이터에서 용매 가열 신호 성분을 미리 제거하거나 정제
[~, y_target_pepced] = HKifuncs.pepc(dat_100ps, heat_100ps_baseline);
...
% Data
cfg.q          = q_fit;
cfg.target_dSq = y_target_pepced;
cfg.target_Std = std_100ps;
cfg.heat_dat   = heat_100ps; % PEPC용 Basis
cfg.heat_dat_baseline = heat_100ps_baseline;  % PEPC용 Basis with baseline
